...
class TestDTWalker:
    ...
    def setup_method(self) -> None:
        ...
        compatible_prop = DTProperty(
            name="compatible",
            value=DTValue.string("test,device")
        )
    ...
    def test_find_properties(self) -> None:
        ...
        result = walker.find_properties(
            lambda prop: prop.value is not None and prop.value.type == DTValueType.INTEGER
        )
        assert len(result) == 1
...
class TestDTMultiWalker:
    ...
    def test_find_nodes_by_compatible_multi_root(self) -> None:
        ...
        with patch.object(self.root1, 'find_nodes_by_compatible', return_value=[self.root1]) as mock1, \
...
class TestBehaviorExtractor:
    ...
    def setup_method(self) -> None:
        ...
        self.hold_tap_node.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("zmk,behavior-hold-tap")
        ))
        
        self.macro_node = DTNode(name="my_macro")
        self.macro_node.add_property(DTProperty(
            name="compatible", 
            value=DTValue.string("zmk,behavior-macro")
        ))
        
        self.tap_dance_node = DTNode(name="my_td")
        self.tap_dance_node.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("zmk,behavior-tap-dance")
        ))
        
        self.generic_behavior_node = DTNode(name="my_behavior")
        self.generic_behavior_node.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("zmk,behavior-custom")
        ))
        
        self.non_behavior_node = DTNode(name="other")
        self.non_behavior_node.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("other,device")
        ))
...
class TestMacroExtractor:
    ...
    def test_extract_macros(self) -> None:
        ...
        macro1.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("zmk,behavior-macro")
        ))
        ...
        non_macro.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("other,device")
        ))
...
class TestHoldTapExtractor:
    ...
    def test_extract_hold_taps(self) -> None:
        ...
        hold_tap1.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("zmk,behavior-hold-tap")
        ))
        ...
        other_behavior.add_property(DTProperty(
            name="compatible",
            value=DTValue.string("zmk,behavior-macro")
        ))
...
class TestComboExtractor:
    ...
    def test_extract_combos_valid(self) -> None:
        ...
        valid_combo.add_property(DTProperty(
            name="key-positions",
            value=DTValue.array([0, 1])
        ))
        valid_combo.add_property(DTProperty(
            name="bindings",
            value=DTValue.string("&kp A")
        ))
        
        # Create invalid combo (missing key-positions)
        invalid_combo1 = DTNode(name="combo2")
        invalid_combo1.add_property(DTProperty(
            name="bindings",
            value=DTValue.string("&kp B")
        ))
        
        # Create invalid combo (missing bindings)
        invalid_combo2 = DTNode(name="combo3")
        invalid_combo2.add_property(DTProperty(
            name="key-positions",
            value=DTValue.array([2, 3])
        ))
    ...
    def test_extract_combos_no_logger(self) -> None:
        ...
        invalid_combo.add_property(DTProperty(
            name="bindings",
            value=DTValue.string("&kp A")
        ))
...
class TestUniversalBehaviorExtractor:
    ...
    def test_extract_behaviors_as_models(self) -> None:
        ...
        with patch('zmk_layout.parsers.ast_behavior_converter.create_ast_behavior_converter', return_value=mock_converter):
    ...
    def test_extract_behaviors_as_models_with_defines(self) -> None:
        ...
        for method_name in [
            'convert_hold_tap_node', 'convert_macro_node', 'convert_combo_node',
            'convert_tap_dance_node', 'convert_sticky_key_node', 'convert_caps_word_node',
            'convert_mod_morph_node', 'convert_input_listener_node'
        ]:
