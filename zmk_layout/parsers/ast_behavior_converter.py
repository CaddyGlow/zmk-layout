...
class ASTBehaviorConverter:
    ...
    def convert_combo_node(self, node: DTNode) -> ComboBehavior | None:
        ...
        try:
            ...
            if not key_positions_prop:
                if self.logger:
                    self.logger.warning(
                        "Combo missing key-positions property", combo_name=name
                    )
                return None
            ...
            if not key_positions:
                if self.logger:
                    self.logger.warning(
                        "Combo has invalid key-positions", combo_name=name
                    )
                return None
            ...
            if not bindings_prop:
                if self.logger:
                    self.logger.warning(
                        "Combo missing bindings property", combo_name=name
                    )
                return None
    ...
    def _populate_combo_properties(self, combo: ComboBehavior, node: DTNode) -> None:
        ...
        if layers_prop:
            ...
            if self.logger:
                self.logger.debug(
                    "Added placeholder layers for combo", combo_name=combo.name
                )
        ...
        try:
            ...
            if self.logger:
                ...
                if prop.value and prop.value.type == DTValueType.ARRAY:
                    self.logger.debug(
                        "Array value present",
                        array_length=len(prop.value.value)
