...
def create_simple_corne_layout():
    ...
    layout.behaviors.add_hold_tap(
        name="hm_a", tap="&kp A", hold="&kp LGUI", 
        tapping_term_ms=280, quick_tap_ms=175
    )
    layout.behaviors.add_hold_tap(
        name="hm_s", tap="&kp S", hold="&kp LALT",
        tapping_term_ms=280, quick_tap_ms=175
    )
    layout.behaviors.add_hold_tap(
        name="hm_d", tap="&kp D", hold="&kp LCTRL",
        tapping_term_ms=280, quick_tap_ms=175
    )
    layout.behaviors.add_hold_tap(
        name="hm_f", tap="&kp F", hold="&kp LSHIFT",
        tapping_term_ms=280, quick_tap_ms=175
    )
    
    # Layer toggle behaviors
    layout.behaviors.add_hold_tap(
        name="lt_spc", tap="&kp SPACE", hold="&mo 1",
        tapping_term_ms=200
    )
    layout.behaviors.add_hold_tap(
        name="lt_ent", tap="&kp ENTER", hold="&mo 2",
        tapping_term_ms=200
    )
    ...
    base_bindings = [
        ...
        "&kp LGUI", "&lt_spc", "&kp LCTRL",
        # Right thumb keys (39-41)
        "&kp LALT", "&lt_ent", "&kp RSHIFT"
    ]
    ...
    lower_bindings = [
        ...
        # Navigation
        "&kp TAB", "&kp HOME", "&kp UP", "&kp END", "&kp PG_UP",
        "&kp PG_DN", "&kp LEFT", "&kp DOWN", "&kp RIGHT", "&kp BSPC",
    ...
    raise_bindings = [
        ...
        "&kp GRAVE", "&kp LBKT", "&kp MINUS", "&kp RBKT", "&kp PIPE",
        "&kp BSLH", "&kp EQUAL", "&kp PLUS", "&kp LBRC", "&kp RBRC",
        
        # More symbols
        "&kp TILDE", "&kp UNDER", "&kp LT", "&kp GT", "&trans",
        "&trans", "&kp SQT", "&kp DQT", "&kp COLON", "&kp QMARK",
    ...
    adjust_bindings = [
        # Media controls
        "&kp C_MUTE", "&kp C_VOL_DN", "&kp C_VOL_UP", "&kp C_PREV", "&kp C_NEXT",
        "&kp C_PP", "&email_macro", "&trans", "&trans", "&trans",
...
def generate_zmk_keymap_file(layout: Layout, output_dir: str = "/tmp/zmk_output"):
    ...
    keymap_content = generate_complete_keymap_file(
        layout_data, behaviors_dtsi, combos_dtsi, macros_dtsi, 
        keymap_node, layer_defines
    )
